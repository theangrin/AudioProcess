...
class ASRExecutor(BaseExecutor):
    def __init__(
        self,
        model_type: str = "conformer_u2pp_online_wenetspeech",
        lang: str = "zh",
    ...
    def _init_from_path(
        ...
        if cfg_path is None or ckpt_path is None:
            ...
            self.ckpt_path = os.path.join(
                self.res_path, self.task_resource.res_dict["ckpt_path"] + ".pdparams"
            )
        ...
        with UpdateConfig(self.config):
            ...
            if "deepspeech2" in model_type:
                self.config.decode.lang_model_path = os.path.join(
                    MODEL_HOME, "language_model", self.config.decode.lang_model_path
                )

                lm_url = self.task_resource.res_dict["lm_url"]
                lm_md5 = self.task_resource.res_dict["lm_md5"]
                self.download_lm(
                    lm_url, os.path.dirname(self.config.decode.lang_model_path), lm_md5
                )
                ...
                if num_decoding_left_chunks:
                    assert (
                        num_decoding_left_chunks == -1 or num_decoding_left_chunks >= 0
        ...
        if "conformer" in model_type or "transformer" in model_type:
            ...
            frame_shift_ms = (
                self.config.preprocess_config.process[0]["n_shift"]
                / self.config.preprocess_config.process[0]["fs"]
            )
            ...
            logger.debug(f"The asr server limit max duration len: {self.max_len}")
    ...
    def preprocess(self, model_type: str, input: Union[str, os.PathLike]):
        ...
        if (
            "deepspeech2" in model_type
            or "conformer" in model_type
            or "transformer" in model_type
        ):
    ...
    def __call__(
        ...
        self.preprocess(self.model_type, audio_file)
        self.infer(self.model_type)
