...
class TextExecutor(BaseExecutor):
    def __init__(
        ...
        if model_type in ["ernie_linear_p7_wudao", "ernie_linear_p3_wudao"]:
            self._init_from_path(task, model_type, lang, config, ckpt_path, punc_vocab)
        else:
            self._init_from_path_new(
                task, model_type, lang, config, ckpt_path, punc_vocab
            )
    ...
    def _init_from_path(
        ...
        if cfg_path is None or ckpt_path is None or vocab_file is None:
            ...
            self.cfg_path = os.path.join(
                self.task_resource.res_dir, self.task_resource.res_dict["cfg_path"]
            )
            self.ckpt_path = os.path.join(
                self.task_resource.res_dir, self.task_resource.res_dict["ckpt_path"]
            )
            self.vocab_file = os.path.join(
                self.task_resource.res_dir, self.task_resource.res_dict["vocab_file"]
            )
        else:
        ...
        if self.task == "punc":
            ...
            self.model = model_class(cfg_path=self.cfg_path, ckpt_path=self.ckpt_path)
    ...
    def _init_from_path_new(
        ...
        if self.task == "punc":
            ...
            if "fast" not in model_type:
                ...
                self.tokenizer = tokenizer_class.from_pretrained("ernie-3.0-mini-zh")
    ...
    def __call__(self, text: str):
        ...
        if self.model_type in ["ernie_linear_p7_wudao", "ernie_linear_p3_wudao"]:
